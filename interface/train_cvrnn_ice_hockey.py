...
def gathering_data_and_run(dir_game, config, player_id_cluster_dir,
    ...
    while True:
        ...
        if training_flag:
            ...
            kld_all, ll_all = train_cvrnn_model(model, sess, config, sample_input_data_t0, sample_target_data_t0,
    ...
    if training_file is not None:
        training_file.write("Mean kl:{0}, Mean ll:{1}\n".format(np.mean(kl_loss_game), np.mean(ll_game)))
        training_file.flush()
    ...
    while True:
        ...
        for dir_game in training_dir_games_all:
            ...
            gathering_data_and_run(dir_game, config,
                                   ...
                                   training_flag=True, game_number=game_number, training_file=training_file)
...
def train_cvrnn_model(model, sess, config, input_data, target_data, trace_lengths, selection_matrix):
    ...
    if print_number == 1:
        print ("acc is {0} with mean KLD:{1} and mean ll:{2} ".format(acc,
                                                                      np.mean(kl_loss),
...
def run():
    ...
    icehockey_cvrnn_config_path = "../environment_settings/" \
                                  "icehockey_cvrnn{0}{2}_config{1}{3}.yaml".format(predicted_target,
                                                                                   ...
                                                                                   predict_action,
                                                                                   embed_mode)
    name_training_record_file = "record_training_cvrnn{0}{2}_config{1}{3}{4}.yaml".format(predicted_target,
                                                                                              box_msg,
                                                                                              predict_action,
                                                                                              embed_mode,
    ...
    with open(name_training_record_file, 'w') as training_file:
        run_network(sess=sess, model=cvrnn, config=icehockey_cvrnn_config, log_dir=log_dir,
                    ...
                    player_id_cluster_dir=player_id_cluster_dir, training_file=training_file)
    sess.close()
